...
public class SnapScene extends ChildView {
    ...
    public <T extends SnapActor> T getActorAt(double aX, double aY, Class<T> aClass)
    {
        for (View child : getChildren()) {
            if (!(child instanceof SnapActor)) continue;
     ...
     * Removes an actor.
     */
    public SnapActor removeActor(int anIndex)
    {
        return (SnapActor) removeChild(anIndex);
    }
    ...
    protected void setShowing(boolean aValue)
    {
        ...
        if (aValue) {
            if (_autoStart) _timer.start(800);
            _autoStart = false;
        }
        else {
     ...
     */
    void startActor(SnapActor anActor)
    {
    ...
    void paintGrid(Painter aPntr)
    {
        ...
        if (_mx > 0 && _my > 0 && _mx < getWidth() && _my < getHeight()) {
            int x = (int) _mx, y = (int) _my;
