...
public class SnapActor extends ImageView {
    ...
    // The actor pen
    private SnapPen _pen = new SnapPen();
    ...
    public int getCellX()
    {
        double cw = getScene() != null ? getScene().getCellWidth() : 1;
        return (int) Math.floor(getX() / cw);
    }
    ...
    public int getCellY()
    {
        double ch = getScene() != null ? getScene().getCellHeight() : 1;
        return (int) Math.floor(getY() / ch);
    }
    ...
    {
        return getAngle(getScene().getX(aName), getScene().getY(aName));
    }
    ...
    {
        return getDistance(getScene().getX(aName), getScene().getY(aName));
    }

    /**
     * Returns the distance to the given actor.
     */
    public double getDistance(SnapActor anActor)
    {
     ...
     * Returns the scene.
     */
    public SnapScene getScene()
    {
        return getParent(SnapScene.class);
    }
