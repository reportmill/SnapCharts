...
public class MapWriter {
    ...
    protected void writeValue(Object anObj)
    {
        if (anObj instanceof String)
            _sb.append('\"').append(anObj).append('\"');
        else if (anObj instanceof Number)
            _sb.append(_fmt.format(anObj));
        else if (anObj instanceof Color)
            writeColor((Color)anObj);
    ...
    protected void writeMap(Map aMap)
    {
        ...
        if (simple && size2<100 && !_compact) {
            delete(size); _compact = true;
            writeMap(aMap); _compact = false;
        }
    }
    ...
    protected void writeList(List aList)
    {
        ...
        for (Object val : aList) {
            ...
            if (simple && (val instanceof Map || val instanceof List))
                simple = false;
        }
        ...
        if (simple && size2<100 && !_compact) {
            delete(size); _compact = true;
            writeList(aList); _compact = false;
        }
    }
    ...
    int getSpaceCount(int anIndex)
    {
        ...
        for (int i=anIndex,iMax=_sb.length(); i<iMax; i++) {
            ...
            if (ws && lastWasWhiteSpace) count++;
            lastWasWhiteSpace = ws;
        }
        return count;
    }
}